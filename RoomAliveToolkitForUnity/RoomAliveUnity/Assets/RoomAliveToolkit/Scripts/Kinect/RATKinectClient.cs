...
namespace RoomAliveToolkit
{
    ...
    public class RATKinectClient : RATSkeletonProvider
    {
        ...
        //by william
        public bool StreamInfrared = false;
        ...
        private FrameRate fpsKinectIR = new FrameRate(1f);  //william
        [Space(10)]
        ...
        /// <summary>
        /// william
        /// The current IR texture received from the server. 
        /// </summary>
        [ReadOnly]
        ...
        //william
        private TCPNetworkStreamer infraredClient;
        protected int infraredPort = 10008;
        ...
        private AutoResetEvent nextInfraredFrameReady = new AutoResetEvent(false);  //by william
        ...
        private string depthPath, rgbPath, audioPath, skeletonPath, depthToCameraTablePath, kinectCalibrationPath, infraredPath; //by william
        private Thread kinectFileStreamerThread = null;
        private IFormatter formatter;
        private Stream depthStream, rgbStream, audioStream, skeletonStream, infraredStream; // by william
        private double streamTime = 0;
        private long pendingDepthTimeStamp, pendingRGBTimeStamp, pendingAudioTimeStamp, pendingSkeletonTimeStamp, pendingIRTimeStamp; //william
        private bool pendingDepthFrameDeserialized, pendingRGBFrameDeserialized, pendingAudioFrameDeserialized, pendingSkeletonFrameDeserialized, pendingIRFrameDeserialized; //william
        ...
        private KinectIRFrame nextIrFrame, currentIrFrame, pendingIrFrame;
        ...
        public void Awake() //guaranteed to be executed before any Start functions
        {
            ...
            //Create a infrared texture //by william
            IRTexture = new Texture2D(depthWidth / 2, depthHeight, TextureFormat.ARGB32, false, true); //ARGB32
            IRTexture.filterMode = FilterMode.Trilinear;            
            IRTexture.Apply();
        ...
        public void Initialize()
        {
            ...
            if (usePlaybackControls)
            {
                ...
                infraredPath = playbackController.streamFolder + name + "_KinectIR.bin";    //william
            }
            ...
            //william
            currentIrFrame = new KinectIRFrame(depthWidth, depthHeight);
            nextIrFrame = new KinectIRFrame(depthWidth, depthHeight);
            ...
            if (StreamToFile)
            {
                ...
                try
                {
                    infraredStream = new FileStream(infraredPath, FileMode.Create);
                }
                catch (Exception)
                {
                    Debug.LogError("Failed to create output file stream for Kinect data to " + infraredPath);
                }
            }
        ...
        public void RestartStreamFromFile()
        {
            ...
            //william
            if (infraredStream != null)
                infraredStream.Close();
        ...
        private void _InitStreamFromFile()
        {
            ...
            if (StreamInfrared)
            {
                try
                {
                    infraredStream = preLoad ?
                    (Stream)(new FileStream(infraredPath, FileMode.Open)) :
                    (Stream)(new MemoryStream(File.ReadAllBytes(infraredPath)));

                    streamStartSample = 0;
                    if (usePlaybackControls && playbackController.trimProCamPlayback)
                        streamStartSample = DepthFrameSize * playbackController.depthFramesTrimFromStart;

                    infraredStream.Seek(streamStartSample, SeekOrigin.Begin);

                    Debug.Log("Opened input stream for Kinect data from " + infraredPath);
                }
                catch (Exception)
                {
                    Debug.LogError("Failed to create input stream for Kinect data from " + depthPath);
            ...
            pendingDepthTimeStamp = pendingRGBTimeStamp = pendingAudioTimeStamp = pendingSkeletonTimeStamp = pendingIRTimeStamp = -1; //william
            pendingDepthFrameDeserialized = pendingRGBFrameDeserialized = pendingAudioFrameDeserialized = pendingSkeletonFrameDeserialized = pendingIRFrameDeserialized = false; //william
            streamTime = 0;

            // Reset frame counters
            numFramesReceived = numFramesSerialized =
                numRGBFramesReceived = numRGBFramesSerialized =
                numFramesRendered = numRGBFramesRendered = numIRFramesRendered = 0;
            ...
            pendingSkeletonFrame = new KinectSkeletonFrame();
            //william
            pendingIrFrame = new KinectIRFrame(depthWidth, depthHeight);
        }
        ...
        private void _RunStreamFromFile()
        {
            ...
            while (runThreads)
            {
                ...
                if (StreamInfrared)
                {
                    if (infraredStream.Position == streamStartSample)
                    {
                        // Streaming just started, get first depth frame
                        pendingIrFrame = (KinectIRFrame)formatter.Deserialize(infraredStream);
                        pendingIRTimeStamp = pendingIrFrame.timeStampIr;
                        pendingIRFrameDeserialized = true;

                        // Set stream time
                        streamTime = pendingIRTimeStamp;
                    }

                    // First handle depth stream:
                    if (pendingIRFrameDeserialized)
                    {
                        // Next depth frame in the stream already acquired
                        if (streamTime >= (double)pendingIRTimeStamp)
                        {
                            lock (nextDepthFrame)
                            {
                                // We have reached the depth frame
                                pendingIRFrameDeserialized = false;
                                nextIrFrame = pendingIrFrame;

                                ++numFramesReceived;
                            }

                            nextInfraredFrameReady.Set();
                        }
                    }

                    if (!pendingIRFrameDeserialized &&
                        infraredStream != null && infraredStream.Position < infraredStream.Length)
                    {
                        // Get the next depth frame
                        pendingIrFrame = (KinectIRFrame)formatter.Deserialize(infraredStream);
                        pendingIRTimeStamp = pendingIrFrame.timeStampIr;
        ...
        public unsafe void Update()
        {
            ...
            else
            {
                ...
                //INFRARED
                if (nextInfraredFrameReady.WaitOne(1))
                {
                    lock (nextIrFrame)
                        Swap<KinectIRFrame>(ref nextIrFrame, ref currentIrFrame);

                    IRTexture.LoadRawTextureData(currentIrFrame.irData); //texture unpacking is done in the shader
                    IRTexture.Apply();

                    ConvertToGrayscale(IRTexture);                   
            ...
            //william
            irFPS = (float)fpsKinectIR.Framerate;
        }

        void ConvertToGrayscale(Texture2D graph)
        {
        ...
        public ShortImage GetIRImage()
        {
            return currentIrFrame.irImage;
        }
        ...
        public void OnApplicationQuit()
        {
            ...
            //william
            if(infraredClient != null)
            {
                infraredClient.ReceivedMessage -= RemoteComputer_NewIRFrame;
                infraredClient.Close();
            }
        ...
        private void RemoteComputer_NewIRFrame(object sender, ReceivedMessageEventArgs args)
        {
            MemoryStream memStream = new MemoryStream(args.data);
            BinaryReader reader = new BinaryReader(memStream);
            ...
            lock (nextIrFrame)
            {
                nextIrFrame.timeStampIr = timeStampIr;
                nextIrFrame.irData = depth;

                Marshal.Copy(depth, 0, nextIrFrame.irImage.DataIntPtr, depth.Length);
                ...
                {
                    // Serialize depth frame to file
                    formatter.Serialize(infraredStream, nextIrFrame);
                    ++numFramesSerialized;
                }
            }
    ...
    //william
    [Serializable]
    public class KinectIRFrame
    {
        public KinectIRFrame(int depthImageWidth, int depthImageHeight)
        {
            irImage = new ShortImage(depthImageWidth, depthImageHeight);

        }

        public long timeStampIr;
        public byte[] irData;


        [NonSerializedAttribute]
        public ShortImage irImage;
